...
class FaceDetectorProcessor(private val view: GraphicOverlay, private val callback: FaceResultCallback?) :
    ...
    override fun onSuccess(results: List<Face>, graphicOverlay: GraphicOverlay, image: Image) {
                ...
                if (isFrontFace(face)) {
                    ...
                    if (isValidFaceSize(face) == FaceSize.VALID) {
                        ...
                        if (isValidEye(face) == EyeStatus.VALID) {
                            // Capture face
//                            val faceCrop = translateCoordinate(face.boundingBox)
    ...
    private fun isValidEye(face: Face): EyeStatus {
        val leftOpenProbability = face.leftEyeOpenProbability
        val rightOpenProbability = face.rightEyeOpenProbability
